___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "GLAMI piXel v2",
  "brand": {
    "id": "glami",
    "displayName": "GLAMI",
    "thumbnail": "data:image/png;base64,\u003d\u003d"
  },
  "description": "GLAMI piXel is the tracking code that unlocks personalization in GLAMI catalogue and assists in delivering better results to e-shops.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "SELECT",
    "name": "tagType",
    "displayName": "Tag type",
    "selectItems": [
      {
        "value": "configuration",
        "displayValue": "Configuration"
      },
      {
        "value": "event",
        "displayValue": "Event"
      },
      {
        "value": "pageView",
        "displayValue": "Page view"
      }
    ],
    "simpleValueType": true,
    "help": "Configuration tag initializes Glami Pixel on a particular page. It should be fired on every page where you want to collect data.",
    "defaultValue": "configuration",
    "alwaysInSummary": true
  },
  {
    "type": "GROUP",
    "name": "configurationSettings",
    "subParams": [
      {
        "type": "TEXT",
        "name": "apiKey",
        "displayName": "API Key",
        "simpleValueType": true,
        "help": "API key which you will receive after shop registration.",
        "textAsList": false,
        "valueHint": "e.g. e2f0b6ed3def5d31612620427b0a93de",
        "valueValidators": [
          {
            "type": "NON_EMPTY",
            "errorMessage": "The value must not be empty"
          }
        ]
      },
      {
        "type": "SELECT",
        "name": "countryCode",
        "displayName": "Country code",
        "macrosInSelect": true,
        "selectItems": [
          {
            "value": "",
            "displayValue": "Select country code..."
          },
          {
            "value": "bg",
            "displayValue": "BG"
          },
          {
            "value": "br",
            "displayValue": "BR"
          },
          {
            "value": "cz",
            "displayValue": "CZ"
          },
          {
            "value": "de",
            "displayValue": "DE"
          },
          {
            "value": "eco",
            "displayValue": "ECO"
          },
          {
            "value": "ee",
            "displayValue": "EE"
          },
          {
            "value": "es",
            "displayValue": "ES"
          },
          {
            "value": "fr",
            "displayValue": "FR"
          },
          {
            "value": "gr",
            "displayValue": "GR"
          },
          {
            "value": "hr",
            "displayValue": "HR"
          },
          {
            "value": "hu",
            "displayValue": "HU"
          },
          {
            "value": "it",
            "displayValue": "IT"
          },
          {
            "value": "lt",
            "displayValue": "LT"
          },
          {
            "value": "lv",
            "displayValue": "LV"
          },
          {
            "value": "pl",
            "displayValue": "PL"
          },
          {
            "value": "ro",
            "displayValue": "RO"
          },
          {
            "value": "ru",
            "displayValue": "RU"
          },
          {
            "value": "si",
            "displayValue": "SI"
          },
          {
            "value": "sk",
            "displayValue": "SK"
          },
          {
            "value": "tr",
            "displayValue": "TR"
          }
        ],
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY",
            "errorMessage": "The value must not be empty"
          }
        ],
        "help": "Glami domain identifier - ISO 3166-1 alpha-2 code"
      },
      {
        "type": "CHECKBOX",
        "name": "sendPageView",
        "checkboxText": "Send a page view event when this configuration loads.",
        "simpleValueType": true,
        "defaultValue": true,
        "enablingConditions": []
      }
    ],
    "enablingConditions": [
      {
        "paramName": "tagType",
        "paramValue": "configuration",
        "type": "EQUALS"
      }
    ],
    "groupStyle": "NO_ZIPPY"
  },
  {
    "type": "GROUP",
    "name": "moreSettings",
    "displayName": "More settings",
    "groupStyle": "ZIPPY_CLOSED",
    "subParams": [
      {
        "type": "GROUP",
        "name": "pageViewSettings",
        "displayName": "",
        "groupStyle": "NO_ZIPPY",
        "subParams": [
          {
            "type": "CHECKBOX",
            "name": "sendEcommerceEvent",
            "checkboxText": "Send a ecommerce event when page view fires.",
            "simpleValueType": true,
            "defaultValue": true
          }
        ],
        "enablingConditions": [
          {
            "paramName": "sendPageView",
            "paramValue": true,
            "type": "EQUALS"
          },
          {
            "paramName": "tagType",
            "paramValue": "pageView",
            "type": "EQUALS"
          }
        ]
      },
      {
        "type": "RADIO",
        "name": "dataSource",
        "displayName": "Data source",
        "radioItems": [
          {
            "value": "dataLayer",
            "displayValue": "Data Layer"
          },
          {
            "value": "customObject",
            "displayValue": "Custom Object",
            "subParams": [
              {
                "type": "SELECT",
                "name": "customObject",
                "displayName": "Ecommerce Object",
                "macrosInSelect": true,
                "selectItems": [],
                "simpleValueType": true,
                "enablingConditions": [
                  {
                    "paramName": "dataSource",
                    "paramValue": "customObject",
                    "type": "EQUALS"
                  }
                ]
              }
            ]
          },
          {
            "value": "manualConfiguration",
            "displayValue": "Manual configuration",
            "subParams": [
              {
                "type": "GROUP",
                "name": "manualConfiguration",
                "displayName": "",
                "groupStyle": "NO_ZIPPY",
                "subParams": [
                  {
                    "type": "SELECT",
                    "name": "eventName",
                    "displayName": "Event name",
                    "macrosInSelect": false,
                    "selectItems": [
                      {
                        "value": "",
                        "displayValue": "Choose event type..."
                      },
                      {
                        "value": "viewContentProduct",
                        "displayValue": "View content (product)"
                      },
                      {
                        "value": "viewContentCategory",
                        "displayValue": "View content (category)"
                      },
                      {
                        "value": "addToCart",
                        "displayValue": "Add to cart"
                      },
                      {
                        "value": "purchase",
                        "displayValue": "Purchase"
                      }
                    ],
                    "simpleValueType": true,
                    "valueValidators": [
                      {
                        "type": "NON_EMPTY",
                        "errorMessage": "Please select an event type",
                        "enablingConditions": [
                          {
                            "paramName": "tagType",
                            "paramValue": "event",
                            "type": "EQUALS"
                          },
                          {
                            "paramName": "sendEcommerceEvent",
                            "paramValue": true,
                            "type": "EQUALS"
                          }
                        ]
                      }
                    ],
                    "enablingConditions": [
                      {
                        "paramName": "tagType",
                        "paramValue": "event",
                        "type": "EQUALS"
                      },
                      {
                        "paramName": "sendEcommerceEvent",
                        "paramValue": true,
                        "type": "EQUALS"
                      }
                    ]
                  },
                  {
                    "type": "GROUP",
                    "name": "viewContentProduct",
                    "displayName": "",
                    "groupStyle": "NO_ZIPPY",
                    "subParams": [
                      {
                        "type": "TEXT",
                        "name": "viewContentProductId",
                        "displayName": "Item ID",
                        "simpleValueType": true,
                        "enablingConditions": [],
                        "valueHint": "e.g. \"AB123\"",
                        "valueValidators": [],
                        "help": "Currently viewed product ID. Use the same ID as you use in the feed (ITEM_ID)"
                      }
                    ],
                    "enablingConditions": [
                      {
                        "paramName": "eventName",
                        "paramValue": "viewContentProduct",
                        "type": "EQUALS"
                      }
                    ]
                  },
                  {
                    "type": "GROUP",
                    "name": "viewContentCategory",
                    "displayName": "",
                    "groupStyle": "NO_ZIPPY",
                    "subParams": [
                      {
                        "type": "TEXT",
                        "name": "viewContentProductIds",
                        "displayName": "Item IDs",
                        "simpleValueType": true,
                        "help": "Currently viewed first 10 product IDs in the category. Use the same IDs as you use in the feed (ITEM_ID).",
                        "valueValidators": [],
                        "valueHint": "e.g. \"AB123,CD345,EF678\""
                      }
                    ],
                    "enablingConditions": [
                      {
                        "paramName": "eventName",
                        "paramValue": "viewContentCategory",
                        "type": "EQUALS"
                      }
                    ]
                  },
                  {
                    "type": "GROUP",
                    "name": "addToCartParams",
                    "displayName": "",
                    "groupStyle": "NO_ZIPPY",
                    "subParams": [
                      {
                        "type": "TEXT",
                        "name": "addToCartProductIds",
                        "displayName": "Item ID",
                        "simpleValueType": true,
                        "help": "IDs of products added into cart. Multiple values are separated by comma (,). Preferably use the same IDs as in your Glami product feed.",
                        "valueHint": "e.g. \"AA123,BB345\"",
                        "valueValidators": []
                      },
                      {
                        "type": "TEXT",
                        "name": "addToCartProductPrice",
                        "displayName": "Product price",
                        "simpleValueType": true,
                        "help": "Total value of products currently added to a cart",
                        "valueHint": "e.g. \"1234.56\"",
                        "valueValidators": []
                      },
                      {
                        "type": "TEXT",
                        "name": "addToCartCurrency",
                        "displayName": "Currency code",
                        "simpleValueType": true,
                        "help": "Products price currency",
                        "valueValidators": [],
                        "valueHint": "e.g. \"EUR\""
                      }
                    ],
                    "enablingConditions": [
                      {
                        "paramName": "eventName",
                        "paramValue": "addToCart",
                        "type": "EQUALS"
                      }
                    ]
                  },
                  {
                    "type": "GROUP",
                    "name": "purchaseParams",
                    "displayName": "",
                    "groupStyle": "NO_ZIPPY",
                    "subParams": [
                      {
                        "type": "TEXT",
                        "name": "transactionId",
                        "displayName": "Order ID",
                        "simpleValueType": true,
                        "help": "Purchase transaction identifier",
                        "valueValidators": [
                          {
                            "type": "NON_EMPTY",
                            "enablingConditions": [
                              {
                                "paramName": "eventName",
                                "paramValue": "purchase",
                                "type": "EQUALS"
                              }
                            ],
                            "errorMessage": "The value must not be empty"
                          }
                        ],
                        "valueHint": "e.g. \"ORDER212\""
                      },
                      {
                        "type": "TEXT",
                        "name": "productIds",
                        "displayName": "Item IDs",
                        "simpleValueType": true,
                        "help": "Bought product IDs. Use the same IDs as you use in the feed (ITEM_ID).",
                        "valueHint": "e.g. \"AB123,CD345,EF678\"",
                        "valueValidators": []
                      },
                      {
                        "type": "TEXT",
                        "name": "productNames",
                        "displayName": "Product names",
                        "simpleValueType": true,
                        "valueHint": "e.g. \"Black lace shoes,Bronze skirt\"",
                        "help": "Names of bought products. Multiple values are separated by comma (,). Preferably use the same names as in your Glami product feed."
                      },
                      {
                        "type": "TEXT",
                        "name": "orderValue",
                        "displayName": "Order value",
                        "simpleValueType": true,
                        "help": "Order value (sum of product values)",
                        "valueHint": "e.g. \"1234.56\"",
                        "valueValidators": [
                          {
                            "type": "NON_EMPTY",
                            "errorMessage": "The value must not be empty",
                            "enablingConditions": [
                              {
                                "paramName": "eventName",
                                "paramValue": "purchase",
                                "type": "EQUALS"
                              }
                            ]
                          }
                        ]
                      },
                      {
                        "type": "TEXT",
                        "name": "currency",
                        "displayName": "Currency code",
                        "simpleValueType": true,
                        "help": "Order currency",
                        "valueValidators": [
                          {
                            "type": "NON_EMPTY",
                            "errorMessage": "The value must not be empty",
                            "enablingConditions": [
                              {
                                "paramName": "eventName",
                                "paramValue": "purchase",
                                "type": "EQUALS"
                              }
                            ]
                          }
                        ],
                        "valueHint": "e.g. \"EUR\""
                      }
                    ],
                    "enablingConditions": [
                      {
                        "paramName": "eventName",
                        "paramValue": "purchase",
                        "type": "EQUALS"
                      }
                    ]
                  }
                ],
                "enablingConditions": [
                  {
                    "paramName": "dataSource",
                    "paramValue": "manualConfiguration",
                    "type": "EQUALS"
                  }
                ]
              }
            ]
          }
        ],
        "simpleValueType": true,
        "defaultValue": "dataLayer",
        "enablingConditions": [
          {
            "paramName": "tagType",
            "paramValue": "event",
            "type": "EQUALS"
          },
          {
            "paramName": "sendEcommerceEvent",
            "paramValue": true,
            "type": "EQUALS"
          }
        ]
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "advancedConfiguration",
    "displayName": "Advanced configuration",
    "groupStyle": "ZIPPY_CLOSED",
    "subParams": [
      {
        "type": "TEXT",
        "name": "trackerName",
        "displayName": "Tracker name",
        "simpleValueType": true,
        "help": "When you want to place multiple pixel codes into one page, each pixel must be identified by unique name. You will provide this name in create call. When there\u0027s only one pixel code, you don\u0027t have to use this identifier"
      }
    ]
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

// require necessary APIs
const injectScript         = require('injectScript');
const setInWindow          = require('setInWindow');
const callInWindow         = require('callInWindow');
const createArgumentsQueue = require('createArgumentsQueue');
const isConsentGranted     = require('isConsentGranted');
const addConsentListener   = require('addConsentListener');
const templateStorage      = require('templateStorage');
const copyFromDataLayer    = require('copyFromDataLayer');
const makeNumber           = require('makeNumber');

// globals
const url                  = 'https://glamipixel.com/js/compiled/pt.js';
const templateVersion      = '2.0';
const source               = 'gtm'+templateVersion;
const trackerName          = data.trackerName ? data.trackerName + '.' : '';

// init arguments queue
const glami = createArgumentsQueue('glami', 'glami.q');
setInWindow('GlamiTrackerObject', 'glami');

// consent, consent change listener
let consent = isConsentGranted('analytics_storage') ? 1 : 0;
addConsentListener(
  'analytics_storage',  
  (consentType, granted) => {
    if(consentType == 'analytics_storage'){
      consent = granted ? 1 : 0;
      glamiPixel.set();
    }
  }
);

// pixel definiction
const glamiPixel = (() => {
  let load = () => {
    if(templateStorage.getItem('glamiPixelLoaded') !== true) {
      injectScript(
        url, 
        () => { templateStorage.setItem('glamiPixelLoaded', true); }, 
        () => { data.gtmOnFailure(); }
      );
    }
  };
  
  let create = () => {
    callInWindow('glami', 'create', data.apiKey, data.countryCode, trackerName, {source: source, consent: consent});  
  };
  
  let set = () => {
    callInWindow('glami', 'set', {consent: consent});  
  };
  
  let trackEvent = (eventName, eventParams) => {
    callInWindow('glami', trackerName + 'track', eventName, eventParams);
  };
  
  
  return {
    load:       load,
    create:     create,
    set:        set,
    trackEvent: trackEvent
  };
})();

//
// business logic below
//

let guaEE={}, ga4Event={}, ga4EventModel={};

if(data.dataSource == 'dataLayer'){
  guaEE            = copyFromDataLayer('ecommerce') || {};
  ga4Event         = copyFromDataLayer('event') || {};
  ga4EventModel    = copyFromDataLayer('eventModel') || {};
} else if (data.dataSource == 'customObject'){
  guaEE            = data.customObject.ecommerce || {};
  ga4Event         = data.customObject.event || {};
  ga4EventModel    = data.customObject.eventModel || {};
}

glamiPixel.load();
if(data.tagType == 'configuration') glamiPixel.create();

let glamiPixelEvents = [];

// pageView
if(data.tagType == 'pageView' || data.sendPageView == true) glamiPixel.trackEvent('PageView', {consent: consent});

// viewContentProduct
if(data.eventName == 'viewContentProduct') glamiPixel.trackEvent('ViewContent', {'consent': consent, 'content_type': 'product', 'item_ids': data.viewContentProductId});
if(guaEE.hasOwnProperty('detail'))         glamiPixel.trackEvent('ViewContent', {'consent': consent, 'content_type': 'product', 'item_ids': (guaEE.detail.products || []).map((elm)=>{return elm.id;})});
if(ga4Event == 'view_item')                glamiPixel.trackEvent('ViewContent', {'consent': consent, 'content_type': 'product', 'item_ids': (ga4EventModel.items || []).map((elm)=>{return elm.item_id;})});

// viewContentCategory
if(data.eventName == 'viewContentCategory') glamiPixel.trackEvent('ViewContent', {'consent': consent, 'content_type': 'category', 'item_ids': data.viewContentProductIds});
if(guaEE.hasOwnProperty('impressions'))     glamiPixel.trackEvent('ViewContent', {'consent': consent, 'content_type': 'category', 'item_ids': (guaEE.impressions.products || []).map((elm)=>{return elm.id;})});
if(ga4Event == 'view_item_list')            glamiPixel.trackEvent('ViewContent', {'consent': consent, 'content_type': 'category', 'item_ids': (ga4EventModel.items || []).map((elm)=>{return elm.item_id;})});

// addToCart
if(data.eventName == 'addToCart') glamiPixel.trackEvent('AddToCart', {'consent': consent, 'item_ids': data.addToCartProductIds, 'value': data.addToCartProductPrice, 'currency': data.addToCartCurrency});
if(guaEE.hasOwnProperty('add'))   glamiPixel.trackEvent('AddToCart', {'consent': consent, 'item_ids': (guaEE.add.products || []).map((elm)=>{return elm.id;}), 'value': (guaEE.add.products || []).reduce((prev,elm)=>{return prev + makeNumber(elm.price);}, 0), 'currency': guaEE.currencyCode});
if(ga4Event == 'add_to_cart')     glamiPixel.trackEvent('AddToCart', {'consent': consent, 'item_ids': (ga4EventModel.items || []).map((elm)=>{return elm.item_id;}), 'value': ga4EventModel.value, 'currency': ga4EventModel.currency});

// purchase
if(data.eventName == 'purchase')     glamiPixel.trackEvent('Purchase', {'consent': consent, 'item_ids': data.productIds, 'value': data.orderValue, 'currency': data.currency, 'transaction_id': data.transactionId});
if(guaEE.hasOwnProperty('purchase')) glamiPixel.trackEvent('Purchase', {'consent': consent, 'item_ids': (guaEE.purchase.products || []).map((elm)=>{return elm.id;}), 'value': (guaEE.purchase.actionField || {}).revenue, 'currency': guaEE.currencyCode, 'transaction_id': (guaEE.purchase.actionField || {}).id});
if(ga4Event == 'purchase')           glamiPixel.trackEvent('Purchase', {'consent': consent, 'item_ids': (ga4EventModel.items || []).map((elm)=>{return elm.item_id;}), 'value': ga4EventModel.value, 'currency': ga4EventModel.currency, 'transaction_id': ga4EventModel.transaction_id});


data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "glami"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "GlamiTrackerObject"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "glami.q"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://glamipixel.com/*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_consent",
        "versionId": "1"
      },
      "param": [
        {
          "key": "consentTypes",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "consentType"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "analytics_storage"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "read_data_layer",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keyPatterns",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "ecommerce"
              },
              {
                "type": 1,
                "string": "event"
              },
              {
                "type": 1,
                "string": "eventModel"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_template_storage",
        "versionId": "1"
      },
      "param": []
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 17.10.2023, 11:49:07


